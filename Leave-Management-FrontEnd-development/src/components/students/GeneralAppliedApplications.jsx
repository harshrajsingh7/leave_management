...
import { Calendar, MapPin, Clock, Badge, Download, FileText } from 'lucide-react'; 
import { Chip, Tooltip, Box, Typography, Avatar, Button, IconButton } from '@mui/material';
...
// Function to generate and download PDF - Fixed to work with jsPDF autotable
const generatePDF = (row) => {
  try {
    // Create a new jsPDF instance
    const doc = new jsPDF();
    const duration = Math.ceil((new Date(row.to_date) - new Date(row.from_date)) / (1000 * 60 * 60 * 24));
    
    // Add header with logo
    doc.setFontSize(18);
    doc.setTextColor(44, 62, 80);
    doc.text("Leave Approval Certificate", 105, 20, { align: "center" });
    ...
    const tableData = [
      ...
      ["From Date", format(new Date(row.from_date), 'dd MMM yyyy')],
      ["To Date", format(new Date(row.to_date), 'dd MMM yyyy')],
    ...
    // Save the PDF
    doc.save(`Leave_Approval_${row.id}.pdf`);
...
const StudentApplication = () => {
  ...
  const fetchStudentApplications = async () => {
    setIsLoading(true);
    try {
      const data = await AxiosPost("fetchStudentApplications.php", {
        reg_no: user.reg_no,
      });
      if (data.success) {
  ...
  // Add columns including the new download button column
  const columns = [
    {
      flex: 0.15,
      field: "from_date",
      ...
      renderCell: ({ row }) => {
        return (
          <Tooltip title={format(new Date(row.from_date), 'PPP')} arrow>
            <Box sx={{ 
              display: "flex", 
              alignItems: "center", 
              gap: 1,
              ...
              <Typography sx={{ 
                ...
                {format(new Date(row.from_date), 'dd MMM yyyy')}
              </Typography>
            </Box>
          </Tooltip>
        );
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        ...
        return (
          <Tooltip title={`Duration: ${duration} days`} arrow>
            <Box sx={{ 
              display: "flex", 
              alignItems: "center", 
              gap: 1,
              ...
              <Typography sx={{ 
                ...
                {format(new Date(row.to_date), 'dd MMM yyyy')}
              </Typography>
            </Box>
          </Tooltip>
        );
